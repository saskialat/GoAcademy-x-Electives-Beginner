...
class Drink < Entry
    ...
    # GIMANA
    def set.temperature
        "Do you want your drink hot or cold? (hot/cold) \n>"
        choice = gets.chomp.downcase
        return choice
    end

    def caffeine?
        "Does #{name} contain caffeine? (1: YES, 2: NO) \n> "
        input = gets.chomp.to_i
        if input == 1
            true
        else
...
class Main 
    ...
    def start
        ...
        # this wasn't used 
        @flavor = Tag_list.new(["sweet", "sour", "bitter", "spicy", "salty", "umami"])
        @nutrition = Tag_list.new(["healthy", "junk_food"])
        @country = Tag_list.new(["japan", "indonesia", "china"])
...
application.start
    ...
    # can we inherit a method only, and not a class?